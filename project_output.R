...
<h2>Clean-up data</h2>
<p>NAs don’t help us with predction. So let’s go ahead and keep columns without missing values.</p>
<pre class="r"><code>training_data &lt;- training_data[, colSums(is.na(training_data)) == 0]
testing_data &lt;- testing_data[, colSums(is.na(testing_data)) == 0]</code></pre>
</div>
<div id="inspet" class="section level2">
<h2>Inspet</h2>
<p>We want to reduce noise and irrelevant variables. Let’s look at the column names and use our judgement about what to keep and what to ignore.</p>
<pre class="r"><code>names(training_data) # the first 7 columns won' be needed</code></pre>
...
<pre class="r"><code>training_data &lt;- training_data[ , -c(1:7)]
testing_data  &lt;- testing_data[ , -c(1:7)]</code></pre>
<p>Preview the restuls, using our judgement about what might be useful. And check to see if there are missing data</p>
<pre class="r"><code>summary(training_data) # looks like all variables have useful summaries</code></pre>
